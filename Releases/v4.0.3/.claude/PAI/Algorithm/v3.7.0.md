...
- **LEARNING CONTEXT ENRICHMENT (Extended+ only):**

  At Extended+ effort, load deeper learning context to inform ISC generation and risk assessment:

  ```bash
  bun run ~/.claude/hooks/lib/learning-readback-cli.ts --depth extended
  ```

  Review the output for:
  1. Failure patterns that might apply to the current task (similar domain, similar mistake category)
  2. Low-rated learnings that reveal recurring user frustrations
  3. Synthesis recommendations that should influence approach

  If any failure pattern or learning is directly relevant to the current task, add a `### Known Risks (from learning history)` subsection to the PRD's `## Context` section citing the specific pattern.

  For Deep/Comprehensive effort, also read the 3 most recent FAILURES CONTEXT.md files in full:
  ```bash
  ls -t ~/.claude/MEMORY/LEARNING/FAILURES/*/*/CONTEXT.md | head -3
  ```
  Then use Read tool on each file's `## What Happened` section for full root cause context.
