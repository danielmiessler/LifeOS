...
**SPRINT CONTRACT (Advanced+ only — skip for Standard/Extended):**

Before BUILD begins, the builder and evaluator must agree on "done." This is the Generator-Evaluator separation pattern (inspired by Anthropic's harness design research): the agent doing the work should never be the sole judge of its own output.

1. **Builder proposes:** For each ISC criterion, state HOW it will be satisfied and HOW it will be tested. Write this to `## Sprint Contract` in the PRD.
2. **Evaluator confirms:** If a QA agent will be used, it reviews the contract and flags any criteria that aren't testable as written. If no QA agent, the primary agent self-reviews with explicit adversarial framing: "If I were trying to ship broken work, which criteria would I rationalize passing?"
3. **Contract locked:** Once confirmed, the contract is the BUILD target. Scope changes require updating the contract first, not silently adjusting.
...
**CALIBRATED EVALUATION (Advanced+ with QA agent, or any build producing user-facing output):**

After binary pass/fail testing, score the work on four dimensions. This catches the gap between "technically works" and "actually good." Scoring is POST-test — it supplements pass/fail, never replaces it. Inspired by Anthropic's generator-evaluator harness design research.

| Dimension | Weight | What It Measures |
|-----------|--------|-----------------|
| **Correctness** | 30% | Does it do what was specified? Every ISC criterion actually satisfied, not rationalized. |
| **Completeness** | 25% | Are there gaps? Missing states, unhandled edges, empty views, stub implementations. |
| **Craft** | 25% | Technical execution quality — code structure, naming, error handling, performance, accessibility. |
| **User Experience** | 20% | Would a real user find this intuitive? Visual coherence, interaction feedback, information hierarchy. |

**Scoring scale:** 1-10 per dimension. Anchors: 1-3 = broken/missing, 4-5 = present but poor, 6-7 = solid/functional, 8-9 = polished/impressive, 10 = best-in-class.

**Threshold gate:** Score below **6** on ANY dimension → targeted feedback specifying what's below bar → redo that dimension only. Do not redo passing dimensions.

Write scores to `## Evaluation Scores` in the PRD:
```markdown
## Evaluation Scores
| Dimension | Score | Evidence |
|-----------|-------|----------|
| Correctness | 8/10 | All 12 ISC criteria verified with tests |
| Completeness | 6/10 | Empty state for search not handled |
| Craft | 7/10 | Clean code, minor naming inconsistency in utils |
| User Experience | 5/10 | Form lacks validation feedback — REDO |
```
...
🧠 LEARNING:
 ...
 [🧠 STRIPPING TEST: Which scaffolding steps were load-bearing (caught real issues, changed outcomes)? Which added latency without catching anything? Flag candidates for removal. Every component in a harness encodes an assumption about what the model can't do on its own — stress test those assumptions.]
