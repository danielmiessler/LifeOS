...
**PRD body expansion uses Edit, not Write (MANDATORY).** The initial stub above is the ONLY single-shot Write of the PRD. Every subsequent body section (Context, Criteria, Decisions, Verification, and any subsections like `### Risks` or `### Plan`) is added via its own Edit call capped at roughly 200 lines of new content. Do not attempt to overwrite the stub with a single large Write containing the full body: long Write calls can be truncated mid-parameter by the tool-call wrapping layer, leaving the file as a frontmatter-only stub on disk while the model believes the content landed. After any Write or Edit that lands more than 50 lines of new prose, immediately Read the first and last 10 lines of the file to confirm the content is on disk. Never embed literal closing-tag strings for system-injected XML (function_calls, parameter, invoke, system-reminder) in PRD content; use HTML entities or paraphrase.
