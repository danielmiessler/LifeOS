...
**1. Thinking-capability floor (HARD, v6.1.0).** At E2+, the count of *thinking* capabilities (from the "Thinking & Analysis Capabilities" section of `capabilities.md` — IterativeDepth, ApertureOscillation, FirstPrinciples, SystemsThinking, RootCauseAnalysis, Council, RedTeam, Science, BeCreative, Ideate, BitterPillEngineering, Evals, WorldThreatModel, Advisor, FeedbackMemoryConsult, ContextSearch, Fabric patterns) is a **hard floor** — it cannot be relaxed via show-your-math. Difficult work earns thinking depth, full stop. Synthesis tasks, comparison tasks, design tasks, doctrine tasks all qualify as difficult and must invoke the thinking floor. A capability "selected" but not invoked is a CRITICAL FAILURE. Selecting fewer thinking capabilities than the tier requires is *also* a critical failure — there is no escape valve.

**2. Delegation-capability floor (SOFT, v6.1.0).** Delegation capabilities (Forge, Anvil, Cato, Agent Teams, Custom Agents, Background Agents, Worktree Isolation, Research, etc.) remain show-your-math relaxable — sometimes the work is genuinely single-author and delegation adds noise. The justification must name the work the un-selected delegation capabilities would have done and why that work isn't needed. "Doesn't seem necessary" is not justification.

**Why this split (v6.1.0).** Field experience showed that "show your math" was being used as a universal escape valve to skip thinking on hard tasks under the cover of "this is just synthesis, I can do it inline." The thinking floor exists *because* the model under-uses thinking capabilities on the exact tasks they're designed for. Making it soft re-creates the regression. v6.1.0 closes the loophole: thinking is structural, not optional.
...
| **Thinking floor (v6.1.0, HARD)** | Thinking-capability count meets the tier hard floor (E1 0-1, E2 ≥2, E3 ≥4, E4 ≥6, E5 ≥8). **Cannot be relaxed via show-your-math.** A "show your math" entry that names fewer thinking capabilities than the tier requires is a doctrine violation, not a justification. |
| **Delegation floor (v6.1.0, soft)** | Delegation-capability count meets the tier soft floor (E2 ≥1, E3 ≥2, E4 ≥2, E5 ≥4). Overridable with "show your math" in `## Decisions` naming the work the un-selected delegation would have done. |
...
- **Thinking floor (v6.1.0, HARD)** — meet the tier thinking floor (E2 ≥2, E3 ≥4, E4 ≥6, E5 ≥8). Cannot be relaxed via show-your-math.
- **Delegation floor (v6.1.0, soft)** — meet the tier delegation floor or document "show your math" in `## Decisions` naming what the un-selected delegation would have done.
