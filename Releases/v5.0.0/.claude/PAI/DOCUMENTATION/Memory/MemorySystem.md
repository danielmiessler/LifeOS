...
- **PAI MEMORY** (`${PAI_DIR}/MEMORY/`) — structured, hook-driven, entity-based
- **Auto-Memory** (`${CLAUDE_CONFIG_DIR}/projects/<project>/memory/`) — unstructured learnings, research findings, contact profiles, reference material — anything Claude captures during sessions
...
**Location:** `${PAI_DIR}/MEMORY/` + `${CLAUDE_CONFIG_DIR}/projects/<project>/memory/`
...
**Location:** `${CLAUDE_CONFIG_DIR}/projects/-Users-{username}--claude/`
_(Replace `{username}` with your system username, e.g., `-Users-john--claude`)_
...
ls -lt ${CLAUDE_CONFIG_DIR}/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail ${CLAUDE_CONFIG_DIR}/projects/-Users-{username}--claude/$(ls -t ${CLAUDE_CONFIG_DIR}/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```
...
- Migrated 2 unique feedback items to CC's native memory at `${CLAUDE_CONFIG_DIR}/projects/-Users-{YourName}--claude/memory/`
...
- Enabled Claude Code's built-in auto-memory using default path (`${CLAUDE_CONFIG_DIR}/projects/<project>/memory/`)
...
- Moved BACKUPS to `${CLAUDE_CONFIG_DIR}/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration

- Previous: `${CLAUDE_CONFIG_DIR}/history/`, `${CLAUDE_CONFIG_DIR}/context/`, `${CLAUDE_CONFIG_DIR}/progress/`
...
**Location:** `${CLAUDE_CONFIG_DIR}/projects/<project>/memory/` (default, matches system prompt injection)
