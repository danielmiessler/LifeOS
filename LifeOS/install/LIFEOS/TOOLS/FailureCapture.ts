...
interface FailureCaptureInput {
  ...
  sessionId?: string;
  /** Provenance of the rating. Only 'explicit' (or unlabeled legacy) lows are real failures. */
...
export async function captureFailure(input: FailureCaptureInput): Promise<string | null> {
  const { transcriptPath, rating, sentimentSummary, detailedContext, sessionId, source } = input;
  ...
  // Provenance gate: an inferred/implicit low rating is a guess, not a real failure.
  // Only explicit (or unlabeled legacy) lows feed failure capture, a prediction-as-truth guard.
  if (source && source !== 'explicit') {
    console.error(`[FailureCapture] Rating source '${source}' is not explicit, skipping (inferred low, not a real failure)`);
    return null;
  }
  ...
  // 1. Write bounded transcript tail (full-transcript copies were the primary
  // disk-bloat source: hundreds of MB across per-message captures)
  const transcriptContent = readFileSync(transcriptPath, 'utf-8');
  const transcriptLines = transcriptContent.split('\n');
  if (transcriptLines[transcriptLines.length - 1] === '') transcriptLines.pop();
  const transcriptTail = transcriptLines.slice(-200).join('\n');
  writeFileSync(join(failureDir, 'transcript.tail.jsonl'), transcriptTail + '\n', 'utf-8');
...
| \`transcript.tail.jsonl\` | Last 200 lines of conversation (bounded) |
