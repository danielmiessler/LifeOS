...
const DEFAULT_CONFIG_PATH = resolve(claudeDir(), "LIFEOS/USER/CONFIG/LIFEOS_CONFIG.toml");
...
export function paiUserDir(): string {
  try {
    ...
    return resolve(claudeDir(), "LIFEOS/USER");
  }
}
...
function validateAndNormalize(raw: unknown, path: string): LifeosConfig {
  ...
  return {
    ...
    paths: {
      userDir: expandHome(
        root.paths?.userDir ?? root.paths?.user_dir ?? resolve(claudeDir(), "LIFEOS/USER"),
      ),
      memoryDir: expandHome(
        root.paths?.memoryDir ?? root.paths?.memory_dir ?? resolve(claudeDir(), "LIFEOS/MEMORY"),
      ),
