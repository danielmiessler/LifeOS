...
- **Screenshots go through `Tools/Capture.sh`, never raw `interceptor screenshot`.** Capture.sh runs the preflight gate, enforces the not-Default target check, handles UUID-rot rebind, prefers the DOM-render path, stages the CLI write in a `mktemp -d` and MOVES it to the destination (no CWD strays), and defaults output to `$LIFEOS_DOWNLOADS_DIR` (fallback `~/Downloads/`).
...
- **Screenshot — how it actually works (0.22.2).** Route every capture through `Tools/Capture.sh`; the raw behavior below is what it wraps.
  ...
  - **`--save` writes to the CLI's current working directory** and returns `filePath` (omits `dataUrl`). It does NOT write to `/tmp/pai-screenshots/` — workflows `cd` there precisely because `--save` targets CWD. There is no `--out`/`--output <path>` flag — unknown flags are SILENTLY SWALLOWED (this is how stray `interceptor-screenshot-*.png` files used to leak into the session CWD; root-caused 2026-07-18). `Tools/Capture.sh` therefore runs the CLI inside a private `mktemp -d` stage and MOVES the result to its `--out` target, so no stray is ever left behind. Its default destination is `$LIFEOS_DOWNLOADS_DIR` (the global artifact-output knob from settings.json's env block; fallback `~/Downloads/`); pipeline intermediates go to `/tmp/`; an unwritable destination fails hard (exit 13), never falls back to CWD.
